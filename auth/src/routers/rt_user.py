...
async def create_user(
    ...
    # item = ops_user.create_user(db, request)
...
async def get_user_list(
    ...
    current_user: SchemaUserDisplay = Depends(get_current_user),
):
...
async def __get_user_list(
    ...
    return await ops_user.get_user_list(
        db, page_index=page_index, page_size=page_size, sort_by=sort_by
    )
...
@router.put(
    "/{id}/password", status_code=status.HTTP_204_NO_CONTENT, summary="Change password"
)
